...
mod test {
	...
	#[test]
	fn test_check_ias_report() {
		let ias_report = "{\"id\":\"242600690142260141766765698309602198256\",\"timestamp\":\"2024-08-01T07:31:18.412928\",\"version\":4,\"epidPseudonym\":\"vGVzYujnJjmi20vaPl6la89YnrH1mLhttwfOAdh6TEf6Qx2ZcKkINjzawuAMx5sy36lA0B/YrBMj/8b+B637If5yaj4XM5/paJttdt5LpORKZUloBNLKrjXfqlZ3MZbJ9TvJUB7W7xpc6l7zCNfvey+WAFBi1tZu8BwCTc5kng0=\",\"advisoryURL\":\"https://security-center.intel.com\",\"advisoryIDs\":[\"INTEL-SA-00334\",\"INTEL-SA-00615\"],\"isvEnclaveQuoteStatus\":\"SW_HARDENING_NEEDED\",\"isvEnclaveQuoteBody\":\"\"}".as_bytes();
		let (sgx_fields, report_timestamp) = SgxFields::from_ias_report(ias_report).unwrap();
		println!("EPID RA report measurement hash  :{:?}",sgx_fields.measurement_hash());
		println!("REPORT TIMESTAMP :{:?}",report_timestamp);
	}
}
